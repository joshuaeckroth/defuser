...
% queries:
% cardString(D, C, "{w#=w#}", []), constraintsToPredicate(C, Pred).
% cardString(D, C, CS, []), constraintsToPredicate(C, [[2, defused, 'w#=w#']]), format("~s~n", [CS]).
% cardString(3, C, CS, []), constraintsToPredicate(C, P), format("~s~n", [CS]).
